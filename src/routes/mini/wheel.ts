...
export async function handleWheelMiniApi(args: WheelArgs): Promise<Response | null> {
  ...
  if (type === "wheel.spin" || type === "wheel_spin" || type === "spin") {
    ...
    // ✅ A) Coins prize => award immediately (no wallet / no wheel_redeems)
if (kind === "coins" && prizeCoins > 0) {
  try {
    await awardCoins(
      ...
      "wheel_prize_coins",
      String(spinId),
      `Wheel coins prize: ${String(prize.code || "")}`,
      `wheel:coins:${(ctx as any).publicId}:${tg.id}:${spinId}:${String(prize.code || "")}:${prizeCoins}`
    );
  } catch (e: any) {
    logWheelEvent({
      code: "mini.wheel.spin.fail.db_error",
      msg: "Failed to award coins for coin prize",
      appPublicId,
      tgUserId,
      route,
      extra: { spinId, error: String(e?.message || e) },
    ...
    await db.prepare(
      `UPDATE wheel_spins
       SET status='claimed',
           prize_code=?,
           prize_title=?,
           ts_claim=datetime('now')
       WHERE id=?`
    ).bind(String(prize.code || ""), String(prize.title || ""), spinId).run();
  } catch (e: any) {
    logWheelEvent({
      code: "mini.wheel.spin.fail.db_error",
      msg: "Failed to update spin status to claimed for coin prize",
      appPublicId,
      tgUserId,
      route,
      extra: { spinId, error: String(e?.message || e) },
  ...
  logWheelEvent({
    ...
    msg: "Spin completed (coin prize claimed)",
    appPublicId,
    tgUserId,
    route,
    extra: { spinId, prizeCode: String(prize.code || ""), prizeCoins },
  });
