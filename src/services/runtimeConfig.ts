...
export function extractRuntimeConfigFromBlueprint(BP: any) {
  ...
  const blocksDict =
    BP && BP.blocks && typeof BP.blocks === "object" ? BP.blocks : {};
  ...
  // Wheel
  const wheel = insts.find(
    (b) => b && (b.key === "bonus_wheel_one" || b.type === "bonus_wheel_one")
  );
  if (wheel) {
    ...
    const arr = Array.isArray(p.prizes)
      ? p.prizes
    ...
    cfg.wheel.prizes = arr
      .map((pr: any) => {
        ...
        // ✅ NEW: cost in coins (for item prizes)
        const costCoinsRaw = Number(pr?.cost_coins ?? pr?.costCoins ?? 0);
        ...
        return {
          ...
          kind,
          coins:
            kind === "coins" && Number.isFinite(cRaw)
              ? Math.max(0, Math.round(cRaw))
              : 0,
          ...
          cost_coins:
            kind === "item" && Number.isFinite(costCoinsRaw)
              ? Math.max(0, Math.round(costCoinsRaw))
              : 0,

          // economics / inventory (legacy; пока оставляем)
          cost_cent:
            kind === "item" && Number.isFinite(costCentRaw)
              ? Math.max(0, Math.round(costCentRaw))
              : 0,
          cost_currency: String(pr?.cost_currency ?? pr?.currency ?? "RUB"),
          cost_currency_custom: String(
            pr?.cost_currency_custom ?? pr?.currency_custom ?? ""
          ),

          track_qty: kind === "item" ? !!trackQty : false,
          qty_left:
            kind === "item" && Number.isFinite(qtyLeftRaw)
              ? Math.max(0, Math.round(qtyLeftRaw))
              : 0,
  ...
  // Styles passport
  const passp = insts.find(
    (b) =>
      b && (b.key === "styles_passport_one" || b.type === "styles_passport_one")
  );
  if (passp) {
    ...
    if (p.passport_key !== undefined)
      cfg.passport.passport_key = String(p.passport_key || "default").trim();
  ...
  if (prof) {
    ...
    if (isFinite(p.coins_per_correct))
      cfg.profile_quiz.coins_per_correct = Number(p.coins_per_correct);
    if (isFinite(p.max_per_submit))
      cfg.profile_quiz.max_per_submit = Number(p.max_per_submit);
  }
