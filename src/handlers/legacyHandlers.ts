...
export async function ensureAppOwner(appId, ownerId, env){
  const row = await env.DB.prepare(
...
export async function listMyApps(env, ownerId, request) {
...
export async function createApp(request, env, url, ownerId) {
...
export async function getApp(appId, env, request) {
  const raw = await env.APPS.get('app:' + appId);
...
export async function saveApp(appId, request, env) {
  const body = await request.json().catch(() => ({}));
...
export async function deleteApp(appId, env, request) {
...
export async function publishApp(appId, env, url, request) {
  const raw = await env.APPS.get('app:' + appId);
...
export async function getPublicConfig(publicId, env, request) {
  const map = await env.APPS.get('app:by_public:' + publicId, 'json');
  if (!map || !map.appId) {
    return json({ ok:false, error:'NOT_FOUND' }, 404, request);
  }
...
export async function saveBotIntegration(appId, env, body, ownerId, request) {
...
export async function listBroadcasts(appId, env, ownerId, request){
  const appPublicId = await getCanonicalPublicIdForApp(appId, env);
...
export async function createAndSendBroadcast(appId, env, ownerId, request){
...
export async function listDialogs(appId, env, ownerId, request){
  const appPublicId = await getCanonicalPublicIdForApp(appId, env);
  if (!appPublicId) return json({ ok:false, error:'APP_PUBLIC_ID_NOT_FOUND' }, 404, request);
...
export async function getDialogMessages(appId, tgUserId, env, ownerId, request){
  const appPublicId = await getCanonicalPublicIdForApp(appId, env);
  if (!appPublicId) return json({ ok:false, error:'APP_PUBLIC_ID_NOT_FOUND' }, 404, request);
...
export async function sendDialogMessage(appId, tgUserId, env, ownerId, request){
  const body = await request.json().catch(()=>null);
  if (!body) return json({ ok:false, error:'BAD_JSON' }, 400, request);
...
export async function getBotIntegration(appId, env, ownerId, request) {
...
// DELETE /api/app/:id/bot  {"action":"unlink"}
// Отключает интеграцию бота для приложения (KV токен + webhook secret + row в D1).
export async function deleteBotIntegration(appId, env, ownerId, request) {
  const appPublicId = await getCanonicalPublicIdForApp(appId, env);
  if (!appPublicId) {
    return json({ ok: false, error: 'APP_PUBLIC_ID_NOT_FOUND' }, 404, request);
  }
  ...
  try {
    await env.DB
      .prepare('DELETE FROM bots WHERE owner_id = ? AND app_public_id = ?')
      .bind(ownerId, appPublicId)
      .run();
  } catch (e) {
...
// (mini api moved to src/routes/mini.ts)
...
// NOTE: /api/cabinet/apps/* (dialogs/broadcasts/analytics) is routed in src/routes/analytics.ts now.
