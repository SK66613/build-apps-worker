...
function pickMsgType(upd: any) {
  ...
  const txt =
    (upd.message && upd.message.text) || (upd.edited_message && upd.edited_message.text) || "";
...
async function bumpBotOutCounters(db: any, { appPublicId, tgUserId, status = null }: any) {
  await db
    .prepare(
      `UPDATE app_users
    ...
    .bind(status, String(appPublicId), String(tgUserId))
    .run();
}
...
export async function handleTelegramWebhook(
  ...
  // ---- LOG: вход запроса (не палим секрет)
  console.log("[TG webhook] IN", {
    publicId,
    hasSecret: !!s,
    ct: request.headers.get("content-type"),
    len: Number(request.headers.get("content-length") || 0),
    path: url.pathname,
  });

  if (!expected || !timingSafeEqual(s, expected)) {
    console.log("[TG webhook] FORBIDDEN", { publicId });
  ...
  try {
    ...
    try {
      ...
      console.log("[TG webhook] BAD_JSON", { publicId, msg: String((e as any)?.message || e) });
    ...
    console.log("[TG webhook] UPDATE", {
      ...
      has_message: !!upd?.message,
      has_callback: !!upd?.callback_query,
      has_inline: !!upd?.inline_query,
      has_pcq: !!upd?.pre_checkout_query,
      has_payment: !!upd?.message?.successful_payment,
    });
    ...
    if (env.BOT_SECRETS && updateId) {
      ...
      if (seen) {
        console.log("[TG webhook] DEDUPE", { publicId, updateId });
        return new Response("OK", { status: 200 });
      }
    ...
    try {
      ...
      if (botTokenEarly) {
        ...
        if (upd && upd.pre_checkout_query) {
          console.log("[TG webhook] BRANCH", { publicId, branch: "stars_pre_checkout" });
        ...
        if (sp) {
          console.log("[TG webhook] BRANCH", { publicId, branch: "stars_success_payment" });
    ...
    if (!chatId || !from) {
      console.log("[TG webhook] NO_CHAT_OR_FROM", { publicId });
      return new Response("OK", { status: 200 });
    }
    ...
    if (!botToken) {
      console.log("[TG webhook] NO_BOT_TOKEN", { publicId });
      return new Response("OK", { status: 200 });
    }
    ...
    if (!ctx || !ctx.ok) {
      console.log("[TG webhook] NO_CTX", { publicId });
      return new Response("OK", { status: 200 });
    }
    ...
    if (upd && upd.callback_query && upd.callback_query.data) {
      console.log("[TG webhook] BRANCH", { publicId: appPublicId, branch: "callback_query" });
      ...
      async function loadSaleAction(saleId: any) {
      ...
      if (data.startsWith("sale_cancel:")) {
        console.log("[TG webhook] BRANCH", { publicId: appPublicId, branch: "sale_cancel" });
      ...
      if (data.startsWith("pin_menu:")) {
        console.log("[TG webhook] BRANCH", { publicId: appPublicId, branch: "pin_menu" });
        ...
        for (let i = 0; i < items.length; i += 2) {
          ...
          const row: any[] = [];
          row.push({
            text: String(a.title || a.style_id),
            callback_data: `pin_make:${saleId}:${String(a.style_id)}`,
          });
          if (b)
            row.push({
              text: String(b.title || b.style_id),
              callback_data: `pin_make:${saleId}:${String(b.style_id)}`,
            });
          kb.push(row);
        }
      ...
      if (data.startsWith("pin_make:")) {
        console.log("[TG webhook] BRANCH", { publicId: appPublicId, branch: "pin_make" });
        ...
        const pinRes = await issuePinToCustomer(
          env.DB,
          appPublicId,
          cashierTgId,
          String(act.customerTgId),
        ...
        try {
          ...
          try {
            await bumpBotOutCounters(env.DB, { appPublicId, tgUserId: String(act.customerTgId) });
          } catch (_) {}
        } catch (e) {
        ...
        await tgSendMessage(
          ...
          botToken,
          String(chatId),
          `✅ PIN отправлен клиенту ${String(act.customerTgId)} для ${
            stTitle ? `“${stTitle}”` : styleId
          }.`,
          {},
          { appPublicId, tgUserId: cashierTgId }
        );
        try {
          await bumpBotOutCounters(env.DB, { appPublicId, tgUserId: cashierTgId });
        } catch (_) {}
    ...
    if (t === "/start" || t.startsWith("/start ")) {
      ...
      if (payload.startsWith("redeem_")) {
        console.log("[TG webhook] BRANCH", { publicId: appPublicId, branch: "redeem_start" });
        ...
        if (!isCashier) {
          ...
          try {
            await bumpBotOutCounters(env.DB, { appPublicId, tgUserId: String(from.id) });
          } catch (_) {}
          return new Response("OK", { status: 200 });
        }

        // 2) найти redeem (wheel)
        ...
        // === если в wheel_redeems нет — пробуем passport_rewards
        if (!r) {
          console.log("[TG webhook] BRANCH", { publicId: appPublicId, branch: "redeem_passport_try" });
          ...
          // пометить паспортный приз redeemed (only issued -> redeemed)
          const updRes = await env.DB.prepare(
            ...
            .bind(String(from.id), Number(pr.id))
            .run();
          ...
          // монетный приз — начисляем монеты ТОЛЬКО после подтверждения кассиром
          if (coins > 0) {
            try {
              const ctx2: any = await resolveAppContextByPublicId(appPublicId, env);
              const appId2 = ctx2?.appId || appId || null;
              ...
              await awardCoins(
                ...
                `passport:redeem:${appPublicId}:${pr.tg_id}:${pr.id}:${coins}`
              );
            } catch (e) {
          ...
          // сбросить штампы (повторяемость паспорта)
          try {
            await env.DB.prepare(
          ...
          try {
            ...
            try {
              await bumpBotOutCounters(env.DB, { appPublicId, tgUserId: String(pr.tg_id) });
            } catch (_) {}
          } catch (_) {}
        ...
        await env.DB.prepare(
          ...
          .bind(String(from.id), Number(r.id))
          .run();
        ...
        try {
          await env.DB.prepare(
            ...
            .bind(String(from.id), appPublicId, Number(r.id))
            .run();
        } catch (_) {}
        ...
        try {
          ...
          try {
            await bumpBotOutCounters(env.DB, { appPublicId, tgUserId: String(r.tg_id) });
          } catch (_) {}
        } catch (_) {}
      ...
      if (payload.startsWith("sale_")) {
        console.log("[TG webhook] BRANCH", { publicId: appPublicId, branch: "sale_start" });
        ...
        if (!rawTok) {
          await tgSendMessage(
            ...
            botToken,
            chatId,
            "⛔️ Этот QR устарел. Попросите клиента обновить QR.",
            {},
            { appPublicId, tgUserId: from.id }
          );
          try {
        ...
        const tokenAppPublicId =
          tokObj && tokObj.appPublicId ? String(tokObj.appPublicId) : appPublicId;
        ...
        if (env.BOT_SECRETS) {
          ...
          try {
            await env.BOT_SECRETS.delete(saleTokKey(token));
          } catch (_) {}
        }
        ...
        try {
          await bumpBotOutCounters(env.DB, { appPublicId: tokenAppPublicId, tgUserId: String(from.id) });
        } catch (_) {}
    ...
    try {
      ...
      if (pendRaw) {
        console.log("[TG webhook] BRANCH", { publicId: appPublicId, branch: "sale_amount" });

        let pend: any = null;
        try {
          pend = JSON.parse(pendRaw);
        } catch (_) {
          pend = null;
        }
        ...
        if (cents == null) {
          await tgSendMessage(
            ...
            botToken,
            chatId,
            "Введите сумму числом (например 350 или 350.50)",
            {},
            { appPublicId, tgUserId: from.id }
          );
          try {
        ...
        const cashbackCoins = Math.max(0, Math.floor((cents / 100) * (cbp / 100))); // 10% от суммы в монетах
        ...
        if (pend.customerTgId && cashbackCoins > 0) {
          ...
          try {
            await bumpBotOutCounters(env.DB, {
              appPublicId: pend.appPublicId || appPublicId,
              tgUserId: String(pend.customerTgId),
            });
          } catch (_) {}
        }
        ...
        try {
          await bumpBotOutCounters(env.DB, { appPublicId: pend.appPublicId || appPublicId, tgUserId: String(from.id) });
        } catch (_) {}
        ...
        try {
          ...
          if (env.BOT_SECRETS && saleId && pend.customerTgId) {
            await env.BOT_SECRETS.put(actionKey, JSON.stringify(actionPayload), {
              expirationTtl: 3600,
            }); // 1 час
          }
    ...
    if (t === "/profile") {
      console.log("[TG webhook] BRANCH", { publicId: appPublicId, branch: "profile" });

      try {
        // cfg (если есть) из KV app:<id>
        const appObj: any = await env.APPS.get("app:" + ctx.appId, "json").catch(() => null);
        const cfg = (appObj && (appObj.app_config ?? appObj.runtime_config ?? {})) || {};

        const state: any = await buildState(env.DB, ctx.appId, appPublicId, String(from.id), cfg || {});
        const lines = [
